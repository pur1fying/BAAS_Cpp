...
void AScreenCap::init() {
    ...
    if(ver == "0") {
        logger->BAASError("Only support emulator with skd version range from [ 21 ] to [ 28 ], current sdk version : " + to_string(sdk));
        throw RequestHumanTakeOver("AScreenCap Version Not Supported");
    }
    filesystem::path p(ASCREENCAP_BIN_DIR);
    p.append(ver).append(arch).append("ascreencap");
    ...
    int w = int(BAASUtil::st2u32(ret_buffer.substr(byte_pointer + 3 * sizeof(uint32_t), sizeof(uint32_t)))); // width
    int h = int(BAASUtil::st2u32(ret_buffer.substr(byte_pointer + 4 * sizeof(uint32_t), sizeof(uint32_t)))); // height
...
void AScreenCap::decompress() {
    ...
    LZ4_decompress_safe(ret_buffer.c_str() + byte_pointer + 5 * sizeof(uint32_t), decompress_buffer, s, decompress_buffer_size);
