...
void NemuControl::click(int x, int y) {
    std::lock_guard<std::mutex> lock(control_mtx);
    nemu_connection->click(x, y);
}

void NemuControl::long_click(int x, int y, double duration) {
    nemu_connection->long_click(x, y, duration);
}

void NemuControl::swipe(int x1, int y1, int x2, int y2, double duration) {
    int step_len;
    double sleep_delay;
