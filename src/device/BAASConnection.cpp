...
void BAASConnection::detect_device()
{
    ...
    if (serial == "auto") {
        if (n_available == 0) {
            logger->BAASCritical(
                    "No available device found, please set the exact serial in the config [ /emulator/serial ]."
            );
            ...
            config->save();
        } else if (n_available == 2 && (available[0] == "127.0.0.1:7555" && available[1] == "127.0.0.1:16384") ||
                   (available[1] == "127.0.0.1:7555" && available[0] == "127.0.0.1:16384")) {
            ...
            logger->BAASCritical(
                    "Multiple devices found, choose the one you want to run and fill in the config [ /emulator/serial ]."
            );
    ...
    if (!(p.first
        ...
        if (port_device.second == 1 && emu_device.second == 0) {
            logger->BAASInfo(
                    "Find LDPlayer device serial pair [ " + port_device.first + " ] and [ " + emu_device.first +
                    " ]"
            );
...
void BAASConnection::adb_connect()
{
    ...
    for (auto &i: d) {
        switch (i.second) {
            ...
            case 2:
                logger->BAASWarn(
                        "Device [ " + i.first + " ] is unauthorized, please accept ADB debugging on your device."
                );
                break;
            default:
...
void BAASConnection::auto_detect_package()
{
    ...
    if (packages.empty()) {
        logger->BAASCritical(
                "No Blue Archive package found in device [ " + serial + " ], please install game first."
        );
        ...
        logger->BAASCritical(
                "Multiple packages found, choose the one you want to run and fill in the config [ /emulator/package_name ]."
        );
...
{
    adb_shell_bytes(
            "am start -a android.intent.action.MAIN -c android.intent.category.LAUNCHER -n " + package + "/" +
            activity
    );
}
...
{
    server = GameServer::package2server(package_name);
    config->replace("/emulator/server", server);
    logger->BAASInfo("GameServer : " + server);
}
...
void BAASConnection::detect_package()
{
    ...
    if (package_name == "auto")
        ...
        if (std::find(packages.begin(), packages.end(), package_name) == packages.end()) {
            logger->BAASCritical(
                    "Package [ " + package_name + " ] not found in device [ " + serial +
                    " ], please install game first."
            );
...
void BAASConnection::auto_detect_language()
{
    ...
    if (languages.size() == 1) {
        ...
        logger->BAASCritical(
                "Multiple languages found, choose the one you want to run and fill in the config [ /emulator/language ]."
        );
