...
void AppearThenDoProcedure::implement(BAAS *baas, BAASConfig& output) {
    ...
    if(possible.is_array()) for(auto &i : possible) {
...
void AppearThenDoProcedure::solve_feature_appear(BAASConfig* feature, bool show_log) {
...
void AppearThenDoProcedure::solve_feature_action_click(BAASConfig *feature) {
    ...
    baas->click(point.first, point.second, count, type, size, interval, pre_wait, post_wait, feature->getString("name"));
