...
JudgePointRGBRangeFeature::JudgePointRGBRangeFeature(BAASConfig *config) : BaseFeature(config) {
    ...
    for(auto &i : j["rgb_range"].items()) {
        ...
        for(auto &j : i.value()["positions"]) {
            if(!j.is_array() || j.size() != 2)throw JudgePointRGBRangeFeatureError("Feature format error, position size should be 2");
            temp_position.emplace_back(make_pair(j[0], j[1]));
            size1++;
        }
        for(auto &j : i.value()["range"]) {
            if(!j.is_array() || j.size() != 6)throw JudgePointRGBRangeFeatureError("Feature format error range size should be 6");
...
bool JudgePointRGBRangeFeature::compare(BAASConfig *parameter, const cv::Mat &image, BAASConfig &output) {
    vector<string> log;
...
double JudgePointRGBRangeFeature::self_average_cost(const cv::Mat &image, const std::string &server, const std::string &language) {
    string server_language = server + "_" + language;
