...
#ifndef BAAS_BAASCONFIG_H
#define BAAS_BAASCONFIG_H
...
public:
    ...
    inline T get(const std::string &key, T default_value) {
        ...
        try {
            ...
            logger->BAASWarn("Key [ " + key + " ] not found. Use default value.");
    ...
    void replace(const std::string &key, T &value) {
        ...
        try {
            ...
            throwValueError("Key [ " + key + " ] not found : " + e.what());
        }
    }
    ...
    [[nodiscard]] inline const BAASLogger* get_logger() const {
        return logger;
    }
...
protected:
    inline nlohmann::json::iterator findByKey(const std::string &key) {
        ...
        if(it == config.end()) throwValueError("Key [ " + key + " ] not found.");
        return it;
    }
    template <typename T>
    ...
    inline void throwValueError(const std::string &desc) {
        throw ValueError("In Config file [ " + path + " ] : \n" + desc);
    }
    ...
    inline void save_modify_history() {
        ...
        in.close();
        j[BAASUtil::getCurrentTimeString()] = modified;
...
extern BAASConfig* static_config;

extern BAASConfig* config_name_change;


#endif //BAAS_BAASCONFIG_H
