...
class TestOcr(unittest.TestCase):
    ...
    def test_multi_thread_ocr(self):
        ...
        self.assertLess(t_multi_thread, t_single_thread, "Multi thread OCR time should be less than single thread OCR time.")

        # compare result
        for i in range(0, thread_count):
            for j in range(0, image_cnt):
                image_path = each_thread_test_image_name[i][j]
                self.assertEqual(self.multi_thread_ret_text[f"{i}_{image_path}"], single_thread_ret_text[f"{i}_{image_path}"], f"Thread {i} Image {image_path} result not equal.")
